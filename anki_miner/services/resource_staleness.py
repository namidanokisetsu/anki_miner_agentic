...
words; for pitch it means the pitch field goes blank. All three are silent, and
all three are fixed the same way — reimport, which is the migration.

This module aggregates the three families' ``stale_enabled_*`` helpers into the
...
from disk (``meta is None``). Frequency and pitch are optional by design —
activation is derived from an enabled source existing — so a user who never
configured them must never be gated. The trigger is narrow on purpose: the user
asked for the source, it is on disk, and an app upgrade broke it.
"""
    ...
    *,
    ...
    Args:
        ...
        families: Restrict the check to these family keys (``"dictionary"``,
            ``"frequency"``, ``"pitch"``). Used by the backfill worker, which
            gates per requested field — a definition-only backfill must not
            abort over a stale pitch index. ``None`` checks all three.
