...
class PitchImportFlow(ModalImportFlowMixin):
    ...
    def _chain_with_new_sources_appended(self, source_ids: list[str]) -> tuple[PitchSourceEntry, ...]:
        ...
        current.extend(PitchSourceEntry(source_id=source_id, enabled=True) for source_id in unique_ids)
        return tuple(current)

    def add_source(self) -> None:
        """Prompt for one or more pitch files and import them sequentially."""
        ...
        file_dialogs.pick_open_files(
            self._parent,
            QCoreApplication.translate("PitchImportFlow", "Choose pitch accent sources"),
    ...
    def _add_source_picked(self, trace_id: str, picker_started: float, chosen: list[str]) -> None:
        ...
        _log_import_picker_return(trace_id, "pitch source", picker_started, "; ".join(chosen))
        if not chosen:
        ...
        def make_worker(source_path: Path) -> ImportWorker:
            return ImportWorker.for_pitch_source(source_path, self._get_config().pitch_root, overwrite=False)

        def format_label(index: int, total: int, source_path: Path, message: str | None) -> str:
            label = tr_format(
                QCoreApplication.translate("PitchImportFlow", "Pitch source %1 of %2: %3"),
                index,
                total,
                source_path.name,
            )
        ...
        def success_note(meta: dict) -> str:
        ...
        def on_finished(result: _ChainedImportResult[Path]) -> None:
            ...
            if len(jobs) == 1 and result.failures and not result.successes:
                self._report_import_issue(
                    QCoreApplication.translate("PitchImportFlow", "The pitch source could not be imported."),
                    result.failures[0][1],
                )
                return
            ...
            if result.successes:
                lines.append(
                    tr_format(
                        QCoreApplication.translate("PitchImportFlow", "Imported %1 pitch sources:"),
            ...
            if result.failures:
                if lines:
                    lines.append("")
                lines.append(QCoreApplication.translate("PitchImportFlow", "Failed:"))
                lines.extend(f"  • {job.name}: {message}" for job, message in result.failures)
            if result.cancelled:
                if lines:
                    lines.append("")
                lines.append(QCoreApplication.translate("PitchImportFlow", "Cancelled before remaining sources."))
            QMessageBox.information(
                self._parent,
                QCoreApplication.translate("PitchImportFlow", "Pitch Sources Added"),
                "\n".join(lines) or QCoreApplication.translate("PitchImportFlow", "Done."),
            )
